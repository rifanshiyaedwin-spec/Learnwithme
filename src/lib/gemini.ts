...
export async function generateChatResponse(
  ...
  const systemInstruction = `You are a LearningWithAI language instructor.
You are teaching the user ${language} at the ${level} level.
Simulate a real interactive audio-style lesson with these rules:
...
- Start from the ${level} level and guide the user step-by-step.
